...
if __name__ == '__main__':
    ...
    params['IMG_HEIGHT'] = 512
    params['IMG_WIDTH'] = 512
    params['posemap_downsample'] = 4
    bs = 16
    shuffle = False
    epoch = 0
    ...
    # model.load_state_dict(torch.load('/versa/kangliwei/motion_transfer/0423-gan/g_epoch_900.pth'))
    model.load_state_dict(torch.load('/versa/kangliwei/motion_transfer/0505-prog-512-gan/g_epoch_100.pth'), strict=True)
    # model.load_state_dict(torch.load('/versa/kangliwei/motion_transfer/0604-gan/epoch_100.pth'), strict=True)
    # model.load_state_dict(torch.load('/versa/kangliwei/motion_transfer/0605-gan/g_epoch_610.pth'))
    # model.load_state_dict(torch.load('/versa/kangliwei/motion_transfer/0428-vgg/epoch_600.pth'), strict=True)
    model.eval()
    ...
    for i, (src_img, y, src_pose, tgt_pose, src_mask_prior, x_trans, src_mask_gt, tgt_face, tgt_face_box, src_face_box) in enumerate(dl):
        ...
        # writer.add_images('agg/epoch%d'%i, torch.cat((src_img, y, out[0]), dim=3)*0.5+0.5)
